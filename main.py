...
def load_conversation_csv(csv_path: Path, tokenizer, max_tokens: int = 8192):
    ...
    CSV schema (database dump format):
        One row per session. Conversation turns are nested as a JSON array
        in the `messages_json` column. Each message has at minimum:
    ...
    with open(csv_path, encoding="utf-8") as f:
        ...
        for row in reader:
            rows.append(row)
    print(f"  Loaded {len(rows)} session row(s)")
    if not rows:
        raise ValueError(f"No rows found in {csv_path}")

    # Take the first session row; expand messages_json into chat turns
    session = rows[0]
    messages_json = session.get("messages_json")
    if not messages_json:
        raise ValueError(
            f"No messages_json column in {csv_path}. "
            f"Columns present: {list(session.keys())}"
        )

    turns = json.loads(messages_json)
    print(f"  Parsed {len(turns)} messages from messages_json")
    ...
    if not messages:
        raise ValueError("No valid messages extracted from messages_json")
    print(f"  Built {len(messages)} chat messages "
